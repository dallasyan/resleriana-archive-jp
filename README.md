...
Generated-mode profile-backed changes persist to the game-root `profile.bin`. At each generated/hybrid proxy-session start, the launcher creates one timestamped backup in `profile-backups` before any profile mutation. `-replay` mode does not create a profile backup or persist replayed/generated profile changes. Currently persisted generated changes include costume selection, party/member and battle-tool edits, character equipment and progression, all equipment-preset edits, tool lock/conversion, Memoria progression, ship-party/support/cannon upgrades, Mana item use, Home configuration/favorites/selection, and exploration start state. If the session-start backup cannot be created, generated-mode profile writes are refused and the failure is logged in `offline-replay.log`.

Character enhancement, rarity enhancement, all three growboard types, level-limit release, and enhancement-reset handlers use Japanese tables bundled under `game-root\progression-master`. Memoria enhancement, tool conversion, and ship-part upgrades use the same Japanese master bundle. The share package contains master tables and the scrubbed starter profile, not capture sessions or personal profile snapshots.
...
Generated `/web_session/token` returns a fresh synthetic UUID-shaped session token. The request has no banner ID and the response contains no article content. Japanese master data defines banner/button configuration and rate/deck categories; `/gacha/list` supplies computed per-card rates, card lists, and wishlist state. The master tables do not enumerate explicit card-pool membership or calculated percentages. Any embedded web article would be fetched separately; no such article endpoint appeared in the captured game API traffic.

For offline gacha cutscene diagnostics, set the installed game's private `offline-session.txt` to a capture folder containing gacha pulls and launch `AtelierReslerianaJapaneseOffline.bat -replay -gacha-timeline-trace`. The optional BepInEx trace logs phase/branch data lookups to `BepInEx\LogOutput.log` and does not change timeline data. Japanese `GachaPerformTimeline` master data contains same-phase/same-condition alternatives that differ only by their timeline asset hash; the runtime selection point still needs validation before enabling rotation.
...
The package handles Japanese subspace ship-party updates, support/cannon selection and upgrades, along with dungeon entry through `/exploration/start` and a safe empty-resource `/exploration/finish` response. Ship upgrades use the active profile and bundled Japanese ship-part costs. Dungeon entry uses the selected `quest_id`, party number, bundled `exploration-routes-jp.json`, and the profile/capture party state. Dungeon movement, dungeon gathering rewards, and dungeon battles remain pending battle-response work.
